...
async function toggleHost() {
  ...
  if (st?.hostState?.enabled) {
    ...
    if (s2?.hostState) {
      hosting.value = true
      if (s2.hostState.code) pairCode.value = s2.hostState.code
      if (s2.hostState.status) status.value = s2.hostState.status
    }
  }
}
...
onMounted(() => {
  const refreshUI = async () => {
    try {
      const st = await props.execute('getState')
      if (st?.hostState) {
        hosting.value = !!st.hostState.enabled
        if (st.hostState.code) pairCode.value = st.hostState.code
        if (st.hostState.status) status.value = st.hostState.status
        hasPeer.value = !!st.hostState.peerConnected
      }
    } catch {}
  }
  refreshUI()
...
<template>
  <div class="panel">
    ...
    <div v-if="hasPeer" class="peer-info">
      <span class="peer-dot"></span><span>主控已连接</span>
    </div>
  </div>
</template>
